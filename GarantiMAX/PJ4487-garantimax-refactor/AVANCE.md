...
**T-13 a T-16 y T-18 completadas:** las cinco especificaciones están en `docs/reglas/` (1.293 líneas). Son el insumo del backend y el camino crítico del proyecto ya está despejado.

**Siguiente tramo sin backend:** T-09 (guardarraíles de linter), T-10 (métricas), T-11 (CI), T-12 (PWA), T-19 (componentes) y la mitad de T-08 que no necesita endpoints (`IndexedDBLocalStore`, `SentryMonitoringProvider`, `RelojDelSistema`). T-17 (línea base sobre el sistema actual) no bloquea a nadie.

**Cuando se entre al backend** (T-S01 en adelante) hay que pedirle al responsable el **nombre exacto del rol del Asesor Farmer**. Autorización para crear `garantimax_db` ya concedida.
...
| T-S02 | `DbContext` propio y base de datos | Nada — **autorización para crear `garantimax_db` ya concedida** (26-08-2026) |
| T-S05 | Autorización base y su documentación | **Nombre exacto del rol del Asesor Farmer** — se le pide al responsable al empezar |
| T-S01 | Esqueleto del servicio .NET y su rama | Nada |
| T-S03 | Modelo de datos de Fase 1 y migración inicial | T-S02 |
| T-S04 | Siembra de datos de prueba | T-S03, y las decisiones de siembra de `esquema-fase1.md` §8 |
| T-S06 a T-S09 | Endpoints de identidad, terreno, gestión y gastos | T-S05 |
