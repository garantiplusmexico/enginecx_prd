...
- [~] Datos de prueba sembrados en las tablas de referencia (salas, vendedores de sala, clientes) para poder construir y probar — el script sale de T-18 (**hecho**, con cuatro salas ficticias). **La carga REAL es T-75**, agregada el 04-09-2026: no era una casilla, era una tarea sin dueño
...
- [ ] **T-74** — Los dos procesos periódicos del servicio *(agregada el 04-09-2026)*
  - Archivos a crear/modificar: en la API, `Services/GarantiMax/Services/{OpenVisitReminder,OverdueWorkNotices}.cs` y su registro en `Program.cs`
  - Criterio de completitud: el recordatorio de **visita abierta a las 3 h** y su cierre al fin del día, más los avisos de **tarea atrasada** y **bitácora pendiente**; cada uno escribe en `notificaciones` con los tipos que T-62 ya declaró, y **corre una sola vez por jornada aunque haya varias instancias**
  - **Por qué aparece tan tarde, y es un fallo del plan.** T-62 se cerró —con razón, su criterio es el lado del cliente: «respetando lo que producen `visitas-abiertas-cron` y `tareas-atrasadas-cron`»— y su nota decía que a los productores «solo les faltaba dónde correr». Nadie comprobó que existieran. No existen. Lo destapó el mapa de integraciones del manual maestro (§10.7, fila 8) al preguntar qué se rompe el día del corte
  - **Es la única regresión del corte que es puro código nuestro.** El cron viejo seguirá corriendo y no verá ninguna visita nueva; el nuestro no está escrito. El recordatorio de las 3 h **desaparece de los dos sistemas a la vez** — y no es «alguien pierde visibilidad» como el resto de esa tabla, es una red de seguridad del asesor que deja de existir (`paridad.md` §4-bis)
  - **La decisión que hay que tomar antes de escribirlos:** un `BackgroundService` corre **una vez por instancia**, así que con dos tareas de ECS son **dos avisos por asesor**. Las salidas son un candado en la base —una fila que se toma con `SELECT … FOR UPDATE SKIP LOCKED`—, o una sola instancia declarada para los procesos, o un planificador externo que llame a un endpoint. Está planteado en `[repo api] doc/los-dos-procesos-periodicos.md`
  - **Las tres reglas ya están escritas** y no hay que inventarlas: el corte de la bitácora es **ayer, nunca hoy** (G-10), el umbral de la visita abierta es el mismo del correo del sistema actual (V-07), y el `CHECK` de `notificaciones.tipo` ya admite los tres tipos desde el 04-09

- [ ] **T-75** — Carga inicial de los catálogos en la base de producción *(agregada el 04-09-2026)*
  - Archivos a crear/modificar: en la API, `Services/GarantiMax/Data/Seed/carga-inicial.sql` (o el formato que se decida) y `doc/como-se-cargan-los-catalogos.md`
  - Criterio de completitud: las **ocho tablas de catálogo** pobladas con datos reales y verificadas contra el sistema actual — `clientes`, `salas`, `sala_vendedores`, `salas_asignacion`, `asesores` con su `jefe_id`, `feriados`, `gasto_categorias` y `proyectos`—; un asesor real inicia sesión, ve **su** cartera y puede hacer check-in
  - **Por qué es una tarea y no una casilla.** El plan lo daba por «resuelto» con una frase —«la carga real la hace el responsable antes del piloto»— y eso no es un plan: no hay mecanismo, no hay dueño y no hay fecha. Lo único que existe es `siembra-desarrollo.sql`, con **cuatro salas inventadas** y una cabecera que dice «SOLO DESARROLLO». *(Lo levantó el responsable el 04-09-2026 preguntando si había módulos para crear salas. No los hay.)*
  - **No contradice «borrón y cuenta nueva», y conviene decirlo.** Esa decisión es sobre el **historial operativo** —visitas, gastos, bitácoras, tareas—, que no migra. Una sala, un vendedor o un feriado **no son historial: son maestros**. No son el pasado, son la realidad de hoy, y copiarlos del sistema actual respeta la decisión en vez de romperla
...
- [x] **T-62** — Notificaciones al asesor
  ...
  - **Lo que falta SÍ es código, y no lo tenía nadie.** *(Corregido el 04-09-2026.)* Esta nota decía que solo faltaba «dónde correr». Es falso: los dos procesos **no existen** — `grep BackgroundService` en el servicio no devuelve nada. T-62 sigue cerrada porque su criterio es el lado del cliente («respetando lo que producen»), pero los productores no tenían dueño en el plan. Ahora son **T-74**
...
| **Catálogos vacíos en la base nueva**: una visita es siempre a una sala, y sin catálogo no hay salas | Alta | **Alto** | **Para construir y probar, datos sembrados** (decisión del 26-08-2026): T-18 entrega el script. **Para producción es T-75**, agregada el 04-09-2026 — antes era una frase sin dueño, y el impacto se subió de Medio a Alto: con el catálogo vacío el asesor **no puede hacer check-in**, que es lo primero que hace. Riesgo residual: el mecanismo de **actualización continua** sigue sin definir, y su gestión es Fase 2 |
...
| ~~¿Cómo se pueblan los catálogos?~~ | **Parcialmente resuelto, y se corrigió el 04-09-2026.** Para construir y probar: datos sembrados (T-18 entrega el script). Para **producción**: era una frase —«la carga real la hace el responsable»— sin mecanismo ni dueño, y ahora es **T-75**. Sigue abierto cómo se **actualizan** en producción; eso sí es Fase 2 |
