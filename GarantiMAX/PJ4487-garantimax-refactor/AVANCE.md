...
**El vertical de identidad está cerrado, y el motor offline tiene sus dos piezas de base.** T-08 (sesión, monitoreo y cableado del contenedor), T-20 a T-24: dominio de identidad, repositorio del perfil, casos de uso de sesión, pantallas y autorización de rutas. **La aplicación se abre, se entra con `SigaWeb` y se ve el perfil que responde el servicio** — la primera cosa demostrable del proyecto. Después llegaron T-25 (contexto de dispositivo), T-29 (dominio de la operación encolada) y T-30 (almacén sobre IndexedDB y cola offline). **353 pruebas en verde**, `tsc -b`, `lint`, `build` y las **cinco** métricas de arquitectura en cero (la quinta es nueva: cero referencias al tier legacy, criterio de completitud de T-24).
...
- **Por dónde continuar:** T-31, el decorador offline y los casos de uso de sincronización — es lo que conecta la cola (ya construida) con los repositorios, y sin él ningún caso de uso puede encolar. Después, el vertical de **visitas**: su mitad de backend está a medias (falta el borrador de visita en curso y los lobbies) y del lado del frontend son T-26, T-27 y T-33 a T-38. Rama activa: `feature/PJ4487-garantimax-refactor-nucleo-asesor` en `siga_alfa` y `feature/PJ4487-garantimax-nucleo-asesor` en `gp_3.0_siga_api`, las dos en el remoto.
- **Contexto importante:**
