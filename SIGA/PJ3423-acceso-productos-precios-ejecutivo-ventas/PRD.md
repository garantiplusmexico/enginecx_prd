...
El problema es de **visibilidad y autonomía**: el Ejecutivo de Ventas necesita consultar productos y precios de sus clientes para su labor comercial, pero el sistema no le expone esa pantalla. La información ya existe en SIGA (una pantalla de productos y precios por distribuidor que hoy ven otros roles, ej. administrador), por lo que se trata de **reutilizarla en modo lectura** para este rol.

El MVP entrega: visualización de solo lectura de productos y precios **limitada a los distribuidores asignados**, con **edición y descarga/exportación bloqueadas**, y gobernada por un **parámetro de configuración por país** que TI/Soporte enciende o apaga. La funcionalidad se construye **para todos los países** pero permanece **apagada por defecto**; se activa donde el negocio lo solicite (inicialmente México).
...
- **Cómo funciona hoy:** los productos y precios de cada distribuidor viven en una pantalla/módulo existente de SIGA, visible para roles administrativos. El Ejecutivo de Ventas tiene ya sus **distribuidores asignados** en SIGA, pero su perfil **no incluye** el acceso a esa pantalla.
...
Permitir que el rol **Ejecutivo de Ventas** consulte en **modo solo lectura** los productos y precios de sus **distribuidores asignados** dentro de SIGA, reutilizando la pantalla existente, con las acciones de edición y de descarga/exportación bloqueadas, y con la funcionalidad **gobernada por un parámetro de configuración por país** administrado por TI/Soporte. La funcionalidad se deja **preparada para todos los países** y su activación es configurable, arrancando activa donde el negocio lo requiera (México) y apagada en el resto.
...
| Administrador SIGA (rol actual) | Rol que hoy gestiona productos y precios; referencia del comportamiento existente (con edición), no afectado por este cambio. |

## 5. Alcance MVP y funcionalidades

| **Funcionalidad** | **Descripción** |
| --- | --- |
| Visualización de productos y precios (solo lectura) | El Ejecutivo de Ventas accede a la pantalla existente de productos y precios en modo consulta, sin capacidades de escritura. |
...
| Bloqueo de descarga/exportación | Se ocultan/deshabilitan las acciones de descarga o exportación de reportes (Excel/PDF/CSV) para este rol. |
| Parámetro de configuración por país | Setting que habilita o deshabilita la funcionalidad por país; apagado por defecto, administrado por TI/Soporte. |
...
- **Descarga/exportación de reportes por el Ejecutivo de Ventas:** excluido explícitamente por el insumo; podría evaluarse en el futuro con controles de auditoría.
...
| RF-04 | Bloqueo de descarga/exportación | Para el rol Ejecutivo de Ventas se deshabilitan/ocultan descarga y exportación de reportes (Excel/PDF/CSV). |
| RF-05 | Parámetro por país | Existe un setting por país que habilita/deshabilita la funcionalidad; deshabilitado, el rol no obtiene el acceso. |
...
| RNF-03 | Trazabilidad | Se registra que el rol accede/consulta productos y precios (auditoría), con nivel de detalle a confirmar con seguridad/operación. |
| RNF-04 | Reutilización/mantenibilidad | Se reutiliza la pantalla existente en modo lectura, evitando duplicar lógica de catálogo de productos/precios. |
| RNF-05 | Consistencia de datos | Los productos y precios mostrados provienen en tiempo real del catálogo existente (misma fuente que ven los roles administrativos). |
| RNF-06 | Manejo de errores/estados | Si el setting está apagado o el ejecutivo no tiene asignados, se muestra un estado/mensaje adecuado sin exponer datos. |
| RNF-07 | Multi-país / configuración | El parámetro es por país y su valor por defecto es "deshabilitado"; activar un país no afecta a los demás. |

## 10. Integraciones y datos

| **Integración / Fuente** | **Uso esperado** |
| --- | --- |
| SIGA — módulo de configuración/settings | Lectura del parámetro por país que habilita/deshabilita la funcionalidad. |
| SIGA — asignación Ejecutivo→Distribuidor | Lectura de la relación existente para filtrar los distribuidores visibles. |
| SIGA — catálogo de productos y precios por distribuidor | Lectura de productos y precios de los distribuidores asignados (misma fuente que hoy ven los roles administrativos). |

**Datos mínimos para operar el MVP:** identificador del Ejecutivo de Ventas y su rol; relación ejecutivo↔distribuidores asignados; país del proyecto/usuario; catálogo de productos y precios por distribuidor; valor del parámetro de configuración por país.

**Esquema de permisos:** el rol Ejecutivo de Ventas **puede leer** productos y precios de sus distribuidores asignados; **no puede** crear, editar, actualizar, descargar ni exportar; **queda bloqueado** cualquier acceso cuando el parámetro del país está deshabilitado o cuando el distribuidor no está asignado. La administración del parámetro queda reservada a TI/Soporte.
...
| Bloqueo de descarga incompleto | Si existen múltiples vías de exportación/impresión, ocultar solo una dejaría fugas de extracción. |
...
| Pantalla reutilizable | La pantalla actual de productos y precios puede operarse en modo solo lectura para el nuevo rol. |
| Mecanismo de settings por país | Existe un mecanismo de configuración por país donde agregar el nuevo parámetro. |
...
| --- | --- |
| Pantalla objetivo | ¿Cuál es exactamente la pantalla/módulo y ruta de productos y precios que se reutiliza en modo lectura? |
| Alcance del bloqueo de descarga | ¿El bloqueo aplica a todos los formatos (Excel/PDF/CSV) e incluye impresión u otras vías de extracción? |
| Parámetro de settings | ¿Nombre/clave técnica del parámetro, dónde se administra y confirmación de default "apagado"? |
| Trazabilidad | ¿Se requiere registro de auditoría de las consultas del rol? ¿Qué campos y retención? |
| Estado sin asignados | ¿Qué se muestra si el Ejecutivo de Ventas no tiene distribuidores asignados? |
| Detalle de precios | ¿Los precios se muestran completos (moneda, impuestos, vigencias) o con alguna restricción de visualización? |
