...
**El vertical de identidad está cerrado, de punta a punta.** T-08 (sesión, monitoreo y cableado del contenedor), T-20 a T-24: dominio de identidad, repositorio del perfil, casos de uso de sesión, pantallas y autorización de rutas. **La aplicación se abre, se entra con `SigaWeb` y se ve el perfil que responde el servicio** — la primera cosa demostrable del proyecto. **285 pruebas en verde**, `tsc -b`, `lint`, `build` y las **cinco** métricas de arquitectura en cero (la quinta es nueva: cero referencias al tier legacy, criterio de completitud de T-24).
...
- **Por dónde continuar:** el vertical de **visitas**. Su mitad de backend está a medias (falta el borrador de visita en curso y los lobbies) y del lado del frontend arranca en T-27 (Mi Día) y T-30 a T-38. Antes conviene cerrar T-25 (contexto de dispositivo), porque las pantallas de visita lo necesitan para cámara y geolocalización. Rama activa: `feature/PJ4487-garantimax-refactor-nucleo-asesor` en `siga_alfa` y `feature/PJ4487-garantimax-nucleo-asesor` en `gp_3.0_siga_api`, las dos en el remoto.
- **Contexto importante:**
