...
| **F7. Catálogo de equivalencias modelo/versión** | Tabla mantenible que empata la nomenclatura del boletín (texto libre, p. ej. "Creta GLS Premium") con la de Quiter. Lo que no logre mapearse se marca para que una persona lo resuelva, y la equivalencia queda aprendida para los periodos siguientes. |
...
| **F12. Detección de incentivo no registrado** | Identifica operaciones cuyo modelo tenía incentivo vigente en el catálogo y que se facturaron **sin ninguna línea de incentivo**. |
...
| **F19. Escalamiento por antigüedad** | Las discrepancias que superen el umbral configurable —**5 días hábiles por defecto**— escalan a la dirección comercial. |
...
Este principio se apoya en un supuesto validado con el negocio: **todo incentivo recuperable está documentado en algún boletín de la marca; no existen incentivos negociados caso por caso.** Por lo tanto, cualquier importe fuera del catálogo vigente es error por definición, y la plataforma no necesita una vía de excepción para incentivos no documentados.
...
| RF-02 | Extracción asistida por IA | Interpretar el documento cargado y proponer una tabla estructurada con modelo, versión, año modelo, precio de lista, bono flexible total, y % y monto de aportación de marca y de dealer. |
...
| RF-08 | Catálogo de equivalencias de modelo y versión | Mantener una tabla que empate la nomenclatura del boletín con la de Quiter, marcar para resolución humana lo que no logre mapearse y persistir la equivalencia resuelta para periodos posteriores. |
| RF-09 | Extracción diaria de ventas | Obtener de AWS Athena, en corrida programada diaria, el reporte de ventas con factura, referencia, fecha de factura, cliente, vendedor, modelo, versión, año modelo, serie, VIN, tipo de operación, destino del bono, código y cuenta del incentivo e importe registrado sin IVA. |
...
| RF-15 | Detección de incentivo omitido | Identificar operaciones facturadas sin línea de incentivo cuyo modelo sí tenía incentivo vigente a su fecha de factura. |
...
| RF-19 | Validación de código y cuenta | Verificar que el código y la cuenta empleados en la captura correspondan al tipo de operación (contado / crédito) y al destino declarado del bono. |
...
| RF-23 | Escalamiento por antigüedad | Escalar a la dirección comercial las discrepancias que superen un umbral configurable de días sin resolver, con **5 días hábiles** como valor por defecto. |
...
| --- | --- |
| **AWS Athena** (dataset de Autocom con la información de Quiter) | **Solo lectura**, mediante usuario restringido provisto por TI de Autocom. Extracción diaria del reporte de ventas con su desglose de incentivos y de las notas de crédito por VIN. Es la dependencia crítica del MVP: sin este acceso el proyecto se detiene. |
...
**Venta (origen: Athena):** número y fecha de factura, referencia, cliente, vendedor, modelo, versión, año modelo, serie, **VIN**, tipo de operación (contado / crédito), institución financiera cuando aplique, destino del bono (precio / enganche / reducción de tasa), precio de lista y total de la factura.

**Línea de incentivo registrada (origen: Athena):** VIN, factura, código de incentivo, cuenta contable, **importe sin IVA** y número de línea dentro del desglose.
...
| --- | --- |
| Retraso o no concreción del usuario de AWS Athena | **Bloqueante sin plan alterno.** El MVP se definió exclusivamente sobre esta vía y la carga manual del export de Quiter quedó descartada. Si el acceso no llega, el proyecto se detiene. Mitigación acordada: la emisión de este PRD es precisamente lo que habilita el alta del usuario por parte de TI de Autocom. |
| Athena no expone todos los campos requeridos | Podría faltar el desglose de la línea de incentivo, el campo de destino del bono, el código y la cuenta contable, o el vínculo de la nota de crédito con el VIN. Reduciría el alcance de las validaciones y obligaría a rediseñar parte del motor. |
...
| --- | --- |
| Acceso a Athena | TI de Autocom generará el usuario con acceso restringido de solo lectura al dataset que contiene la información de Quiter, una vez que tenga visibilidad de este PRD. |
...
| --- | --- |
| Datos — Athena | ¿Qué tablas o vistas del dataset contienen el reporte de ventas, el desglose de la línea de incentivo y las notas de crédito? ¿Existe diccionario de datos? Se resuelve al recibir el usuario. |
| Datos — Athena | ¿Con qué latencia se actualiza el dataset respecto a Quiter, y en qué ventana horaria conviene correr la extracción diaria? |
| Datos — Athena | ¿Las tablas están particionadas por fecha, de modo que las consultas puedan acotarse para controlar el costo de escaneo? |
| Reglas — captura | ¿Existe un catálogo cerrado de códigos de incentivo y cuentas contables por tipo de operación (contado / crédito) y destino del bono? Es insumo requerido para RF-19. |
| Reglas — captura | ¿Quiter distingue explícitamente el destino del bono (precio / enganche / reducción de tasa), o solo permite inferirlo del tipo de operación? De ello depende poder elegir la lista correcta del anexo. |
