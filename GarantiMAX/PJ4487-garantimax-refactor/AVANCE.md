...
**Pendiente del backend, para el vertical de visitas:** el borrador de visita en curso y los lobbies.

**Pendiente del frontend, sin dependencias:** T-12 (PWA), T-19 (componentes) y `SentryMonitoringProvider` — lo único que queda de T-08, porque el monitoreo hoy va a la consola. T-17 (línea base sobre el sistema actual) sigue esperando acceso a producción, y **conviene no dejarla para el final: después del corte no hay con qué comparar**.
...
- **Por dónde continuar:** el vertical de **visitas**, que es lo único que falta para tener la aplicación de terreno funcionando de punta a punta. Del lado del backend faltan el borrador de visita en curso y los lobbies; del lado del frontend son T-26 (layout adaptativo), T-27 (rutas con carga bajo demanda), T-32 (indicador de sincronización, que ya tiene su caso de uso listo) y T-33 a T-38. Rama activa: `feature/PJ4487-garantimax-refactor-nucleo-asesor` en `siga_alfa` y `feature/PJ4487-garantimax-nucleo-asesor` en `gp_3.0_siga_api`, las dos en el remoto.
- **Contexto importante:**
