...
- [x] **T-11** — Integración mínima en el sitio `autoexplora-alfa`: consumir banners P1 desde la API de Strapi (preview en `dev`, published en prod).
...
- [x] **T-13** — Content type **Article** (título, slug, autor, hero, cuerpo enriquecido, estado, fecha). *(completada 2026-07-24, rediseñada 2026-07-27 — ver AVANCE.md)*
  - Archivos: `src/api/article/**`, `src/components/article/**`.
  ...
  - **Rediseño 2026-07-27:** `content` cambió de campo `blocks` (editor de Bloques nativo) a **Dynamic Zone** de componentes (`article.paragraph/heading/quote/image/list/embed`) — ver la decisión de T-14 abajo, que forzó este cambio retroactivo.

- [x] **T-14** — Editor de texto enriquecido con soporte de imágenes, videos y embeds (YouTube/redes). *(completada 2026-07-27, con pivote de arquitectura — ver AVANCE.md)*
  - Archivos: `src/components/article/**` (componentes de la Dynamic Zone).
  - Criterio (RF-09): inserción de párrafo/encabezado/cita/imagen/lista/embed funcionando, en cualquier orden, con Spanish-friendly UI. ✅ *(recortado: sin barra de formato inline — bold/itálica dentro de un párrafo — porque el sitio actual tampoco la soporta; sin "nota al pie", descartada por decisión del programador)*
  - **Hallazgo crítico (2026-07-27):** el editor de **Bloques nativo de Strapi** fue intentado primero (decisión previa del 2026-07-24), incluyendo un bloque personalizado "Embed" registrado vía `addRichTextBlocks` (API pública, solo admin). **Se descubrió un límite duro del framework**: el validador server-side del campo `blocks` (`@strapi/core/.../entity-validator/blocks-validator.js`) rechaza cualquier `type` de nodo fuera de un enum cerrado y no extensible (`paragraph/heading/quote/list/image/code`) — el error real al guardar es `"Block node is of invalid type"`. No existe ninguna API pública ni oculta para extender ese enum; solo sería posible con un parche no soportado (`patch-package`). Se abandonó por completo el enfoque de bloque personalizado dentro de `blocks`.
  - **Decisión final (confirmada por el programador, 2026-07-27):** `content` se rediseñó como **Dynamic Zone** de componentes (mismo patrón ya usado y aprobado en Banner/T-07) — sin límite de tipos, sin parches, con la UI nativa de "+" de Strapi para insertar cualquier bloque en cualquier posición. El componente `article.embed` (campo `url` único) resuelve el proveedor (YouTube/Instagram/Twitter-X/TikTok/Facebook) por patrón de URL en el momento de renderizar, no al guardar — pendiente implementar esa detección también en el sitio (T-15).
  - **"Nota al pie" descartada del alcance** (confirmado por el programador, 2026-07-27): no existe hoy en el sitio ni en el CMS; si un post la necesita, quien redacta la aproxima a mano con texto (ej. "dato (1)" + un párrafo final "(1) fuente"), igual que se resolvió el autor de las quotes en la decisión anterior — aunque finalmente `article.quote` sí terminó con un campo `author` real (ver nota abajo), a diferencia de lo decidido el 2026-07-24.
  - **Autor de la quote — decisión revisada:** el 2026-07-24 se decidió que el autor de una cita se escribiera como texto dentro de la cita, para evitar construir un bloque personalizado solo para eso dentro del editor de Bloques nativo. Al pivotear a Dynamic Zone (donde cada bloque YA es un componente 100% personalizado), agregar un campo `author` opcional a `article.quote` no tuvo costo adicional — se agregó, preservando con más fidelidad el `<cite>` que ya soporta `BlogPostLayout.tsx` en el sitio actual.

- [x] **T-15** — Consumo de blog en el sitio (listado + detalle) desde Strapi. *(completada 2026-07-28 — ver AVANCE.md)*
  - Criterio: artículos publicados renderizan en el sitio; embeds se muestran correctamente. ✅
  - **Requisito previo:** `autoexplora-alfa` había divergido de `dev` (ya se lanzó a producción como `post-lanzamiento`, con commits ajenos al CMS). Se mergeó `dev` (ya sincronizada con `post-lanzamiento`) hacia `feature/PJ5040-cms-autoexplora-mvp` antes de empezar, para construir sobre el código real de producción.
  - **Hallazgos técnicos (2026-07-28):** (1) `populate[hero]=*` falla en Strapi 5.50.2 para un campo `media` simple (`ValidationError: Invalid key related at hero.related`) — hay que usar `populate[hero]=true`, verificado con curl directo antes de escribir el código final. (2) En `status=draft` (preview), Strapi **siempre** devuelve la versión borrador de cada entrada, cuyo `publishedAt` es `null` sin importar si ya existe una versión publicada — el `sort=publishedAt:desc` nativo de Strapi no ordena nada en preview (todo empata en null). Se resuelve con `createdAt` como respaldo de fecha y reordenando en el cliente (`lib/server/strapiApi.ts`) en vez de confiar en el sort de la API. En producción (`status=published`) no aplica — cada entrada devuelta siempre tiene `publishedAt` real.
  - **Token de solo lectura del sitio:** el token `autoexplora-alfa-site-read` solo tenía permisos de Banner. Se automatizó vía un nuevo bootstrap idempotente en el CMS (`src/bootstrap/ensureSiteApiTokenPermissions.ts`, mismo patrón que `ensureEditorPermissions.ts`) que le agrega `api::article.article.find`/`.findOne` — se aplica solo con reiniciar Strapi, sin tocar el admin a mano en ningún ambiente.
  - **Verificado en vivo:** build de producción (`npm run build`) generó estáticamente `/blog/[slug]` para un slug real de Strapi (SSG vía `generateStaticParams`); el programador publicó un artículo real y un banner en borrador desde el admin, ambos aparecieron correctamente en el sitio local sin cambios adicionales.
...
| `STRAPI_API_TOKEN` (en el sitio, server-only) | Token de lectura hacia Strapi (custom, solo `api::banner.banner.find`/`.findOne`) | Dev / Prod |
